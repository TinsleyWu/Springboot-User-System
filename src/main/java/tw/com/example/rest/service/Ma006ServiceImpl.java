...
import tw.com.example.rest.vo.Ma006ResponseBody;
import tw.com.example.rest.vo.Ma006ResponseHeader;
import tw.com.example.rest.vo.Ma006ResponseLeaves;

@Service
public class Ma006ServiceImpl {
	...
	public Ma006Response vacationSearch(Ma006Request request) {
		
		Ma006Response response = new Ma006Response();
		
		// 組成 Response 的 Header
		Ma006ResponseHeader header= new Ma006ResponseHeader();
		...
		// 組成 Response 的 Body
		Ma006ResponseBody body= new Ma006ResponseBody();
			...
			if(LoginData.isPresent()) {
				...
				if (now.compareTo(LoginData1.getEffectiveTime()) <= 0) {
					...
					if (staffData.isPresent()) {
						...
						List<Ma006ResponseLeaves> leaveList = new ArrayList<>();
						List<LeaveDtlEntity> entityList = leaveDtlRepository.findByPkIden(staffData1.getIden());
						...
						if (entityList.size() == 0) {
							...
							for(int i = 0 ; i < entityList.size(); i++) {
								if (year == entityList.get(i).getPk().getYear().intValue()) {
									...
									Ma006ResponseLeaves leave = new Ma006ResponseLeaves();
									...
									leave.setBeginTime(entityList.get(i).getLeaveBeginTime());
									leave.setEndTime(entityList.get(i).getLeaveEndTime());
									leave.setCreTime(entityList.get(i).getCreTime());
									leave.setRemark(entityList.get(i).getRemark());
